...
test('README arranges six project modules in two columns and three rows', async () => {
  const readme = await readFile(new URL('README.md', root), 'utf8');
  assert.ok(readme.includes('PROJECT BACKPLANE · 06 LINKED MODULES'));
  assert.equal((readme.match(/<img width="410" src="https:\/\/utkucaglar\.github\.io\/utkucaglar\/assets\/port-\d{2}[^\"]+\.svg"/g) ?? []).length, 6);
